...
let updateInvoice = async (req, res) => {
  const FINISHED = 3;
  let { fk_table, total_price, final_food_arr, arr_id_food, fk_id_invoice } =
    req.body;
  ...
  await pool.execute(`update tablefood set status = ${FINISHED} where id = ?`, [
    fk_table,
  ]);
  if (row) {
    res.status(200).json({
...
let getFoodNeedUpdate = async (req, res) => {
  const [rows] = await pool.execute("select * from food where id = ? limit 1", [
    req.body.id,
  ]);
...
let orderFood = async (req, res) => {
  ...
  let [rows] = await pool.execute(
    "insert into invoice_table(id,name,time_eat) values(?,?,?)",
    [v4(), name, time_eat]
  );
...
let addHistoryBooking = async (req, res) => {
  ...
  let [rows] = await pool.execute(
    "insert into history_booking set content = ?, id_invoice=?",
    [content, id_invoice]
  );
  if (rows) {
...
let getDataChart = async (req, res) => {
  let [rows] = await pool.execute(
    "SELECT invoice_table.time_eat, sum(invoice_detail.total_price) as total, invoice_table.name FROM invoice_detail,invoice_table where invoice_detail.fk_id_invoice = invoice_table.id group by invoice_table.name"
  );
